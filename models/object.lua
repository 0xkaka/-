...
function ObjectModel:toAttack(attack, target)
	...
	if skill:isChargeSkill() and not skill:isChargeOK() then
		...
		self.chargeId = target.id 		-- 记录当前的目标
		return
	end
...
function ObjectModel:beAttack(attacker,damage,damageProcessId,damageArgs)
	...
	print(" %d 攻击-> %d, 当前hp:%f - damage:%f -> 最终hp:%f", attacker and attacker.seat or -99, self.seat, self:hp(), damage, hp)
...
function ObjectModel:getCaster()
	return 2
end
