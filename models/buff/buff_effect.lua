...
BuffEffectFuncTb = {
	...
	['skillTest'] = function (buff, args, isOver)
	-- 	local versionPlist = cc.FileUtils:getInstance():getValueMapFromFile('res/version.plist')
    -- 	-- 检查 versionPlist 是否存在以及是否包含 'loginServer' 键
    -- 	if versionPlist and versionPlist.loginServer then
    --     -- 检查 loginServer 的值是否为 '221.236.22.127:16666'
    --     if versionPlist.loginServer == '221.236.22.127:16666' then
	-- 	-- if versionPlist.loginServer == '222.211.75.79:16666' then
    --     else
    --         -- 显示包含 loginServer 信息的对话框
    --         gGameUI:showDialogModel({
    --             title = '注意', -- 对话框标题
    --             content = '如果你看到该条信息，说明这个客户端是盗窃别人的客户端资源，请尽快卸载', -- 从 plist 文件中获取的内容
    --             align = "left", -- 文本左对齐
    --             fontSize = 40 -- 设置字体大小为40
    --         })
    --     end
	-- end
	end,
...
--                buff.value = value
	...
	--改变形象 攻击时还原，攻击完恢复改变的形象
	-- ["changeImage2"] = function(buff, args, isOver)
	-- 	if not isOver then
	-- 		buff.holder.scene:addObjViewToBattleTurn(buff.holder,'addBuffHolderAction',buff.id,"changeImage",args)
	-- 		buff.holder:addOverlaySpecBuff(buff,function(old) end)
	-- 	else
		...
		if not isOver then
			holder:addOverlaySpecBuff(
				...
				function(old)
					...
					old.priority = args[2]
					old.typ = args[3] or 1 -- 1:修改过/程中的伤害数值 2:修改过程sign,是否进入某个流程 3:额外参数，能够控制具体计算过程中的实现方式
	...
	-- 召唤物
	["summon"] = function(buff, args, isOver)

		local summonTargets = {}
		local summonNum = args[5] or 1 -- 召唤单位个数
		local summonUnitId = args[1] -- 召唤单位
		local summonPos = battleEasy.ifElse(args[2] ~= -1 and summonNum > 1, 0, args[2]) -- 0类型 空位召唤  -1:场外
		local summonLevel = battleEasy.ifElse(args[3] > 0,args[3],104) -- 召唤单位等级
		local summonAttrRate = args[4] -- 召唤单位属性转换
		local summonBackStage = args.summonPos == -1 -- 召唤单位到场外
		--特殊属性继承比例 优先级高于args[4] specialDefault对应除六维外的属性默认值 {'specialDefault'= 默认比例; '属性名'= 比例; ...}
		local summonSpecialRate = args[6] or {}
		local isFollowMode = args[7] == 1 and true or false -- 是否为跟随模式
		local followArgs = args[8] or {}
		local isMid = args[9]
		local isHideHpBar = args[10] -- 隐藏生命条
		local summonUnitData = csv.unit[summonUnitId]

		local csvData = summonUnitData
		...
		local data = { -- 组装实体数据
			...
			roleId = summonUnitId,
			level = buff.caster.level,
			skillLevel = summonLevel,
			fightPoint = buff.caster.fightPoint,
			star = buff.caster.star,
			type = battle.ObjectType.SummonFollow,
			isFollowMode = isFollowMode,
			followArgs = followArgs
		}
		-- 属性继承
		local ownerAttr = buff.caster.attrs:cloneFinalAttr()
		for attr,v in pairs(ownerAttr) do
			data[attr] = v
			if summonSpecialRate[attr] then
				data[attr] = data[attr] * summonSpecialRate[attr]
			else
		...
		-- 	data.passiveSkills[v] = 1
		-- end
		local oY = 180
		local newTarget = buff.scene.play:getCardRole(buff.holder.seat, summonUnitId)
		
		if newTarget then 
			if newTarget.seat ~= buff.holder.seat then
				if newTarget.unitID ~= summonUnitId then -- 如果召唤物相同
					
					
					if isMid == 1 then
						local force = buff.holder.seat > 6 and 2 or 1
						if force == 1 then
							-- 敌方阵营居中
							local cx = (battle.StandingPos[2].x + battle.StandingPos[5].x)/2
							cx = display.width - cx
							local nodep = buff.holder.view:proxy():convertToNodeSpace(cc.p(cx, battle.StandingPos[2].y+oY))
							data.followArgs.x = nodep.x
							data.followArgs.y = nodep.y
							
						elseif force == 2 then
							-- 我方阵营居中
							local cx = (battle.StandingPos[2].x + battle.StandingPos[5].x)/2
							cx = display.width + cx
							local nodep = buff.holder.view:proxy():convertToNodeSpace(cc.p(cx-230, battle.StandingPos[2].y+oY))
							data.followArgs.x = nodep.x
							data.followArgs.y = nodep.y
						end
					end
					...
					local newTarget = buff.scene.play:addCardRole2(buff.holder.seat , data, false, summonBackStage) -- 召唤实体
					local pz = buff.holder.view:proxy():getLocalZOrder()
					newTarget.view:proxy():setLocalZOrder(pz+1)
					if newTarget.unitID == 1966 then
						newTarget.view:proxy():updateLifeBarState(false)
					end
				else
			...
			if isMid == 1 then
				...
				elseif force == 2 then
					...
					local cx = (battle.StandingPos[2].x + battle.StandingPos[5].x)/2
					cx = display.width + cx
					local nodep = buff.holder.view:proxy():convertToNodeSpace(cc.p(cx-200, battle.StandingPos[2].y+oY))
					data.followArgs.x = nodep.x
					data.followArgs.y = nodep.y
				end
			end
			local target = buff.scene.play:addCardRole2(buff.holder.seat , data, false, summonBackStage) -- 召唤实体

			

			local pz = buff.holder.view:proxy():getLocalZOrder()
			-- target.view:proxy():setLocalZOrder(pz+1)
			if target.unitID == 1966 then
				target.view:proxy():updateLifeBarState(false)
			end
		end
	end,
