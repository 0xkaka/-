...
function BuffModel:getDamageRecord()
	return self.damageRecord
end
function BuffModel:setDamageRecord(damage)
...
-- 生命值条件
function BuffModel:onHpValue(valueType, val, compOpt)
	-- local beHit = 0
	-- local ret = false
	-- local damageValue
	...
	-- local curHp = math.floor(self.holder:hp()) -- 当前生命值
	-- -- local perHp = math.floor(self.holder:hpMax() * val) -- 最大生命值百分比
	-- local hp = curHp / self.holder:hpMax() -- 当前剩余生命值百分比
	-- -- local lostHp = 1 - hp -- 损失血量百分比
	-- if hp < 0.9 and hp > 0.8 then return true end
	-- if hp < 0.8 and hp > 0.7 then return true end
	-- if hp < 0.7 and hp > 0.6 then return true end
	-- if hp < 0.6 and hp > 0.5 then return true end
	-- if hp < 0.5 and hp > 0.4 then return true end
	-- if hp < 0.4 and hp > 0.3 then return true end
	-- if hp < 0.3 and hp > 0.2 then return true end
	-- if hp < 0.2 and hp > 0.1 then return true end
	-- return ret
	local damageCount
	local ret = false
	local damageValue
	...
	if valueType == 1 then
		...
		local curValue = self.holder:hpMax() * val -- 触发血量
		if damageValue > curValue then return true end
		if damageValue < curValue then 
			damageCount = damageCount + damageValue
		end
