...
function Gate:getCardRole(seat, unitId)
	return self.scene:getObjectByUnitId(seat, battle.ObjectType.SummonFollow, unitId) -- 通过seat以及召唤物类型获取存活单位
end




function Gate:addCardRole2(seat, roleData, onlyDelDead, backStageForce)
	local old = self.scene:getObjectBySeatExcludeDead(seat) -- 通过seat获取存活单位
	local oldExtra = self.scene:getObjectBySeatExcludeDead(seat, battle.ObjectType.SummonFollow) -- 通过seat以及召唤物类型获取存活单位
	local isFollowMode = roleData.isFollowMode -- 召唤物是否跟随模式
	-- if isFollowMode and oldExtra then return end -- 如果存在跟随召唤物则直接返回否则继续执行

	local force = seat > 6 and 2 or 1 -- 如果seat > 6说明是敌方阵营 否则 己方
	if isFollowMode then -- 如果是跟随模式则召唤到外场
...
function Gate:getSpeedRankArrayDeduplication()
	...
	for _, obj in ipairs(array) do
		if not obj then
			...
			if not added[obj.seat] and obj.seat and obj.seat <= self.ObjectNumber then
...
function Gate:printTable(t, indent)
    indent = indent or 0
