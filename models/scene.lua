...
function SceneModel:getExtraHeros(force)
	local heros = {}
	local minSeat = 1
	local maxSeat = 6
	if not force then
	...
	for _, obj in self.extraHeros:order_pairs() do
		if obj.seat <= maxSeat and obj.seat >= minSeat then
			table.insert(heros, obj)
		end
	end
	return heros
end
...
-- 通过unitID获取单位
function SceneModel:getObjectByUnitId(seat, type, unitId)
	local maps
	type = type or battle.ObjectType.Normal
	if type == battle.ObjectType.SummonFollow then -- extraHeros
		maps = self.extraHeros
	...
	for _, obj in maps:order_pairs() do
		if obj.unitID == unitId and obj.seat == seat then
			return obj
		end
	end
end
