...
function globals.mirrorSeat(seat)
	return mirrorTb[seat]
end

function input.object2(id, type)
	id = self.scene.play.operateForce == 2 and mirrorSeat(id) or id
	local object = self.scene:getObjectBySeatExcludeDead(self.seat, battle.ObjectType.SummonFollow)
	env.force = object and object.force or env.force
	env.selectObj = object
	return {object}
end
...
function input.allEx()
	local force = self.seat > 6 and 2 or 1
	local values = self.scene:getExtraHeros(force)
	-- env.force = force
	return values
end
function input.allEx2()
	local values = self.scene:getExtraHeros(2)
	-- env.force = force
	return values
end
...
-- 选择召唤物
function process.selectObjSeat(unitID, targets)
	local ret = {}
	for _, obj in ipairs(targets) do
		if obj.unitID == unitID then
...
-- 自己加的
function process.buffOverlayCount(id, buffs, comp, count)

end

-- 自己加的
function process.sortBuffRecord(id, buffs, count)
